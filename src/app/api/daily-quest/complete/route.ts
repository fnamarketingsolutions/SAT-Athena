...
export async function POST(req: Request) {
  ...
  const baselineRw =
    user.startComposite != null
      ? Math.round(user.startComposite / 2)
      : (user.currentReadingWriting ?? 400);
  const baselineMath =
    user.startComposite != null
      ? user.startComposite - baselineRw
      : (user.currentMath ?? 400);

  const rwScore =
    rwTotal > 0 ? scalePracticeRwScore(rwCorrect) : baselineRw;
  const mathScore =
    mathTotal > 0 ? scalePracticeMathScore(mathCorrect) : baselineMath;
