...
/**
 * Scale practice performance to a section score using full SAT section
 * lengths as the denominator so a handful of correct answers cannot
 * max out at 800.
 */
export const SAT_RW_SECTION_QUESTIONS = 54;
export const SAT_MATH_SECTION_QUESTIONS = 44;

export function scalePracticeRwScore(correct: number): number {
  return scaleRwScore(correct, SAT_RW_SECTION_QUESTIONS);
}

export function scalePracticeMathScore(correct: number): number {
  return scaleMathScore(correct, SAT_MATH_SECTION_QUESTIONS);
}

export function isRwSubject(subject: string): boolean {
  return subject === "reading-writing" || subject === "english";
}
