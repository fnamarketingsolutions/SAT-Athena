...
export async function getStuckPoints(userId: string): Promise<StuckPoint[]> {
  ...
  const [sessionsRes, recentQuestsRes] = await Promise.all([
    supabase
      .from("quiz_sessions")
  ...
  const recentQuests = recentQuestsRes.data ?? [];
  const sessionIds = sessions.map((s) => s.id);
  const questIds = recentQuests.map((q) => q.id);
  const sessionSubtopicMap = new Map(sessions.map((s) => [s.id, s.subtopic_id]));

  const [answersRes, questProblemsRes] = await Promise.all([
    sessionIds.length > 0
      ? (supabase as any)
          .from("quiz_answers")
    ...
    questIds.length > 0
      ? supabase
          ...
          .select("subtopic_id, is_correct, response_time_ms")
  ...
  const questProblems = questProblemsRes.data ?? [];

  if (answers.length === 0 && questProblems.length === 0) return [];
  ...
  for (const p of questProblems) {
    const subtopicId = p.subtopic_id;
    if (!subtopicId) continue;
    ...
    if (!p.is_correct) s.withWrong++;
    if (p.response_time_ms) s.responseTimes.push(p.response_time_ms);
  }
...
export async function getEngagementSummary(userId: string): Promise<EngagementSummary> {
  // Quiz time (SAT practice sessions + daily quests)
  const [quizSessionsRes, dailyQuestsRes] = await Promise.all([
    supabase
      .from("quiz_sessions")
  ...
  const totalQuizTimeSeconds =
    (quizSessionsRes.data ?? []).reduce(
      (sum, s) => sum + (s.time_elapsed_seconds ?? 0),
      0
    ) +
    (dailyQuestsRes.data ?? []).reduce(
      (sum, q) => sum + (q.time_elapsed_seconds ?? 0),
      0
    );
  ...
  const totalUserSessions =
    (userSessions ?? []).length + (dailyQuestsRes.data ?? []).length;
  const avgTutorEntriesPerSession = totalUserSessions > 0
    ? Math.round((tutorsUsed / totalUserSessions) * 100) / 100
    : 0;

  // Improvement trend: compare accuracy of first half vs second half of activity
  const [recentSessionsRes, recentQuestsRes] = await Promise.all([
    supabase
      .from("quiz_sessions")
